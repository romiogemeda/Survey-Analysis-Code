  ...
  if (!activeSurvey) {
    return (
      <div className="card-padded text-center py-12">
        <p className="text-surface-500">Select a survey from the dropdown above to upload data.</p>
      </div>
    );
  }
      ...
      <div className="card-padded">
        ...
        <div
          ...
          className={`border-2 border-dashed rounded-xl p-12 text-center transition-colors ${
            isDragging ? "border-brand-400 bg-brand-50" : "border-surface-300 hover:border-surface-400"
          }`}
        >
          <div className="flex flex-col items-center gap-3">
            <div className="w-14 h-14 rounded-2xl bg-surface-100 flex items-center justify-center">
              <svg width="24" height="24" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1.5} className="text-surface-500">
            ...
            <div>
              ...
              <p className="text-xs text-surface-400 mt-1">Supports .json and .csv files</p>
            </div>
            <label className="btn-secondary cursor-pointer mt-2">
              <input type="file" accept=".json,.csv" multiple onChange={handleFileSelect} className="hidden" />
              Browse Files
            </label>
          </div>
        </div>
            ...
            {results.map((r, i) => (
              <div key={i} className="flex items-center justify-between py-2.5 px-3 rounded-lg bg-surface-50">
                ...
                <div className="flex items-center gap-4 text-sm">
                  <span className="text-surface-500">{r.total_records} total</span>
                  <span className="badge-high">{r.valid_records} valid</span>
                  {r.total_records - r.valid_records > 0 && (
                    <span className="badge-low">{r.total_records - r.valid_records} invalid</span>
                  )}
                </div>
              </div>
            ))}
...
{`[
  {"age": "25", "started_at": "2025-01-15T09:00:00Z",
   "completed_at": "2025-01-15T09:12:00Z", "satisfaction": "4"},
  {"age": "42", "started_at": "2025-01-15T10:00:00Z",
   "completed_at": null, "satisfaction": null}
]`}
            </pre>
          </div>
          <div>
            ...
            <pre className="text-xs font-mono bg-surface-900 text-emerald-400 p-3 rounded-lg overflow-x-auto">
{`age,started_at,completed_at,satisfaction
25,2025-01-15T09:00:00Z,2025-01-15T09:12:00Z,4
42,2025-01-15T10:00:00Z,,`}
            </pre>
          </div>
        </div>
      </div>
